...
@Service	//bean으로 관리하기 위해.
public class MemberServiceImpl implements MemberService{
	//서비스는 Dao 에 의존한다.
	@Autowired
	private MemberDao dao;
	
	//회원 목록을 인자로 전달받은 ModelAndView 객체에 담는 로직을 처리하는 서비스 메서드
	@Override
	...
	//인자로 전달되는 새로운 회원 정보를 DB에 저장하는 비즈니스 로직 처리
	@Override
	public void addMember(MemberDto dto) {
		dao.insert(dto);
	}
	//회원 한 명의 정보를 ModelAndView 객체에 담는 비즈니스 로직 처리
	@Override
