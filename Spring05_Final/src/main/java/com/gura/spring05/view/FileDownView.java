...
public class FileDownView extends AbstractView{
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request,
			HttpServletResponse response) throws Exception {	// download.jsp에서 비즈니스 로직 처리는 파일의 목적에 맞지 않기 때문에  여기서 응답을 하게 한다. 
		...
		//응답 헤더 정보 설정
		response.setHeader("Content-Disposition","attachment;filename="+encodedName);	//encodedName으로 파일 이름을 알려줘서 웹 브라우저가 파일을 만들어내는 것이다.
		response.setHeader("Content-Transfer-Encoding", "binary");
		
		//다운로드할 파일의 크기 읽어와서 다운로드할 파일의 크기 설정
		response.setContentLengthLong(dto.getFileSize());	//파일의 크기를 알려준다. 
